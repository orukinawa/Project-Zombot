...
//		}
	...
	void Update ()
	{
		...
		movementDirection = mGamePadInput.GetAxis(GamePadInput.AxisType.LEFT_STICK_X) * Vector3.right + mGamePadInput.GetAxis(GamePadInput.AxisType.LEFT_STICK_Y) * Vector3.forward;
		...
		//}
		if(mAnimator != null)mAnimator.SetFloat("Speed", movementDirection.sqrMagnitude);
		...
		if(!isDashing)
		{
			...
			facingDirection = new Vector3(mGamePadInput.GetAxis(GamePadInput.AxisType.RIGHT_STICK_X), 0.0f, mGamePadInput.GetAxis(GamePadInput.AxisType.RIGHT_STICK_Y));
			//facingDirection.y = 0;
			
			if(facingDirection.sqrMagnitude == 0.0f) facingDirection = movementDirection;			
