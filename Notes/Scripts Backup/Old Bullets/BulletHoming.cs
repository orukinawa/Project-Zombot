...
public class BulletHoming : BulletNormal
{
	protected Transform target = null;
	public float homingUpdateRate;
	protected float homingUpdateRateTimer = 0.0f;
...
//		Debug.DrawLine(transform.position, transform.position + forward*targetDetectionRadius, Color.blue);	
	...
	public override void _Update ()
	{
		if(target != null)
		{
			if(homingUpdateRateTimer < homingUpdateRate)
			{
				homingUpdateRateTimer += Time.deltaTime;
			}
			else
			{
				...
				rotationStep = ((initialDistance - currDistance)/initialDistance) * maxTurnSpeed;
				if(rotationStep > 0)
				{
